...
describe('GeoConversionError', () => {
    describe(TEST_CATEGORIES.CONSTRUCTOR, () => {
        it('should create error with all parameters', () => {
            const error = new GeoConversionError('Conversion failed', 'Coordinate', 'GeoJSON', new Error('Cause'));
        ...
        it('should create error without cause', () => {
            const error = new GeoConversionError('Conversion failed', 'Coordinate', 'GeoJSON');
            expect(error.message).toBe('Conversion failed');
            expect(error.sourceType).toBe('Coordinate');
            expect(error.targetType).toBe('GeoJSON');
    ...
    describe('Usage Patterns', () => {
        it('should work in try-catch block', () => {
            expect(() => {
                throw new GeoConversionError('Conversion failed', 'Coordinate', 'GeoJSON');
            }).toThrow(GeoConversionError);
        });
        it('should maintain error chain with cause', () => {
            const cause = new Error('Original error');
            const error = new GeoConversionError('Conversion failed', 'Coordinate', 'GeoJSON', cause);
    ...
    describe(TEST_CATEGORIES.ERROR_HANDLING, () => {
        ...
        it('should handle undefined cause', () => {
            const error = new GeoConversionError('Conversion failed', 'Coordinate', 'GeoJSON', undefined);
            expect(error.cause).toBeUndefined();
        });
        it('should handle non-Error cause', () => {
            const error = new GeoConversionError('Conversion failed', 'Coordinate', 'GeoJSON', 'Not an error');
