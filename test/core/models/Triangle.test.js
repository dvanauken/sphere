...
import { assertDistanceNearlyEqual, assertAngleNearlyEqual } from '../../__helpers__/assertions.js';
import { LONDON, PARIS, NEW_YORK, TRIANGLE_VERTEX_1, TRIANGLE_VERTEX_2, TRIANGLE_VERTEX_3 } from '../../__fixtures__/coordinates.js';
import { EARTH_RADIUS } from '../../__fixtures__/distances.js';
import { EQUILATERAL, RIGHT_ANGLE } from '../../__fixtures__/angles.js';
...
describe('Triangle', () => {
    describe(TEST_CATEGORIES.CONSTRUCTOR, () => {
        ...
        it('should throw error for invalid vertex count', () => {
            expect(() => {
                new Triangle([TRIANGLE_VERTEX_1, TRIANGLE_VERTEX_2]);
            }).toThrow();
        });
    });
    describe('Area Calculations', () => {
        ...
        it('should calculate area of equilateral triangle', () => {
            ...
            const vertices = angles.map(angle => {
                ...
                const lon = Math.atan2(Math.sin(angle.toRadians()) *
                    Math.sin(radius.inMeters() / EARTH_RADIUS.inMeters()) *
                    Math.cos(center.latitude * Math.PI / 180), Math.cos(radius.inMeters() / EARTH_RADIUS.inMeters()) -
    ...
    describe('Perimeter Calculations', () => {
        it('should calculate perimeter', () => {
            ...
            const sumOfSides = new Distance(sides.reduce((sum, side) => sum + side.inMeters(), 0));
    ...
    describe('Angle Calculations', () => {
        it('should calculate angles', () => {
            ...
            const sumOfAngles = angles.reduce((sum, angle) => sum + angle.degrees, 0);
        ...
        it('should calculate right angle in right triangle', () => {
            ...
            const hasRightAngle = angles.some(angle => Math.abs(angle.degrees - 90) < 1);
    ...
    describe(TEST_CATEGORIES.EDGE_CASES, () => {
        it('should handle degenerate triangles', () => {
            ...
            const vertices = [0, 120, 240].map(lon => new Coordinate(0, lon));
            const triangle = Triangle.from(vertices[0])
                .to(vertices[1])
                .and(vertices[2]);
            const area = triangle.area();
        ...
        it('should handle triangles crossing date line', () => {
            const triangle = Triangle.from(new Coordinate(0, 179)).to(new Coordinate(0, -179)).and(new Coordinate(1, 180));
            const area = triangle.area();
            expect(area).toBeGreaterThan(0);
            expect(area).toBeLessThan(1000); // Should be small
        });
        it('should handle triangles including poles', () => {
            const triangle = Triangle.from(new Coordinate(90, 0) // North pole
            ).to(new Coordinate(0, 0) // Equator at prime meridian
            ).and(new Coordinate(0, 90) // Equator at 90°E
            );
            const area = triangle.area();
            expect(area).toBeGreaterThan(0);
            const angles = triangle.angles();
    ...
    describe('String Representation', () => {
        it('should format triangle correctly', () => {
            const triangle = Triangle.from(TRIANGLE_VERTEX_1)
                .to(TRIANGLE_VERTEX_2)
                .and(TRIANGLE_VERTEX_3);
            expect(triangle.toString()).toBe(`Triangle(${TRIANGLE_VERTEX_1} → ${TRIANGLE_VERTEX_2} → ${TRIANGLE_VERTEX_3})`);
        });
    });
});
