...
import { LONDON, PARIS, NEW_YORK, TOKYO, NORTH_POLE, SOUTH_POLE, EQUATOR_PRIME, QUAD_1, QUAD_2, QUAD_3, QUAD_4 } from '../../__fixtures__/coordinates.js';
import { NORTH, EAST, SOUTH, WEST } from '../../__fixtures__/angles.js';
...
describe('Azimuth', () => {
    ...
    describe('Forward Azimuth', () => {
        it('should calculate forward azimuth between major cities', () => {
            ...
            testCases.forEach(({ start, end, expectedDegrees }) => {
                ...
                assertAngleNearlyEqual(azimuth.forward(), new Angle(expectedDegrees), 1);
            });
        });
        it('should calculate cardinal directions from equator', () => {
            ...
            assertAngleNearlyEqual(Azimuth.from(EQUATOR_PRIME).to(north).forward(), NORTH);
            assertAngleNearlyEqual(Azimuth.from(EQUATOR_PRIME).to(east).forward(), EAST);
            assertAngleNearlyEqual(Azimuth.from(EQUATOR_PRIME).to(south).forward(), SOUTH);
            assertAngleNearlyEqual(Azimuth.from(EQUATOR_PRIME).to(west).forward(), WEST);
        });
    });
    ...
    describe('String Representation', () => {
        it('should format azimuth correctly', () => {
            const azimuth = Azimuth.from(LONDON).to(PARIS);
            expect(azimuth.toString()).toBe(`Azimuth(${LONDON} → ${PARIS})`);
        });
    });
});
