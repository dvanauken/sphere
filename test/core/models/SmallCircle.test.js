...
import { assertDistanceNearlyEqual, assertAreaNearlyEqual } from '../../__helpers__/assertions.js';
import { LONDON, EQUATOR_PRIME, NORTH_POLE, CIRCLE_CENTER } from '../../__fixtures__/coordinates.js';
import { ONE_KILOMETER, TINY_DISTANCE } from '../../__fixtures__/distances.js';
...
describe('SmallCircle', () => {
    ...
    describe('Sphere Customization', () => {
        it('should use custom sphere radius for calculations', () => {
            ...
            assertDistanceNearlyEqual(Sphere.getRadius(customRadius), customRadius);
        });
    });
    describe('Circumference Calculations', () => {
        it('should calculate circumference for various radiuses', () => {
            ...
            testCases.forEach(({ radius, expected }) => {
                const circle = SmallCircle.withCenter(EQUATOR_PRIME)
                    .radius(radius);
                assertDistanceNearlyEqual(circle.circumference(), expected, radius.inMeters() * 0.01 // 1% tolerance
                );
            });
        });
    ...
    describe('Area Calculations', () => {
        it('should calculate area for various radiuses', () => {
            ...
            testCases.forEach(({ radius, expected }) => {
                const circle = SmallCircle.withCenter(EQUATOR_PRIME)
                    .radius(radius);
                assertAreaNearlyEqual(circle.area(), expected, expected * 0.01 // 1% tolerance
                );
            });
        });
    });
    describe('Point Generation', () => {
        it('should generate specified number of points', () => {
            ...
            points.forEach(point => {
                const distance = GreatCircle.from(CIRCLE_CENTER)
                    .to(point)
                    .distance();
                assertDistanceNearlyEqual(distance, ONE_KILOMETER, ONE_KILOMETER.inMeters() * 0.01 // 1% tolerance
                );
            });
        });
        it('should generate evenly spaced points', () => {
            ...
            for (let i = 0; i < points.length; i++) {
                ...
                distances.push(GreatCircle.from(points[i])
                    .to(nextPoint)
                    .distance()
                    .inMeters());
            }
    ...
    describe('Latitude Effects', () => {
        it('should maintain consistent properties at different latitudes', () => {
            ...
            const circles = testLatitudes.map(lat => SmallCircle.withCenter(new Coordinate(lat, 0)).radius(radius));
    ...
    describe('String Representation', () => {
        it('should format small circle correctly', () => {
            const circle = SmallCircle.withCenter(CIRCLE_CENTER)
                .radius(ONE_KILOMETER);
            expect(circle.toString()).toBe(`SmallCircle(center: ${CIRCLE_CENTER}, radius: ${ONE_KILOMETER})`);
        });
    });
});
