...
import { LONDON, PARIS, NEW_YORK, TOKYO, EQUATOR_PRIME, NORTH_POLE } from '../../__fixtures__/coordinates.js';
import { EARTH_RADIUS, LONDON_TO_PARIS, ZERO_DISTANCE } from '../../__fixtures__/distances.js';
...
describe('Arc', () => {
    describe(TEST_CATEGORIES.CONSTRUCTOR, () => {
        it('should create arc on sphere with default radius', () => {
            ...
            assertDistanceNearlyEqual(circumference, Distance.fromKilometers(2 * Math.PI * 6371), 1000);
        });
        it('should create arc on sphere with custom radius', () => {
            const radius = Distance.fromKilometers(1000);
            const arc = Arc.onSphere(radius);
            const circumference = arc.length();
            assertDistanceNearlyEqual(circumference, Distance.fromKilometers(2 * Math.PI * 1000), 1);
        });
        ...
        it('should create arc with central angle', () => {
            ...
            const length = arc.length();
            assertDistanceNearlyEqual(length, Distance.fromKilometers(Math.PI * 6371 / 2), 1);
        });
    });
    describe('Length Calculations', () => {
        ...
        it('should calculate length of quarter earth circumference', () => {
            ...
            assertDistanceNearlyEqual(arc.length(), Distance.fromKilometers(10000), // ~10,000 km (quarter Earth circumference)
            100);
        });
        it('should calculate length of major city pairs', () => {
            ...
            testCases.forEach(({ start, end, expectedKm }) => {
                ...
                assertDistanceNearlyEqual(arc.length(), Distance.fromKilometers(expectedKm), 100);
            });
        });
    });
    ...
    describe(TEST_CATEGORIES.EDGE_CASES, () => {
        it('should handle antipodal points', () => {
            const arc = Arc.fromPoints(new Coordinate(0, 0), new Coordinate(0, 180));
            assertDistanceNearlyEqual(arc.length(), Distance.fromKilometers(20000), // ~20,000 km (half Earth circumference)
            100);
        });
        it('should handle tiny arcs', () => {
            const start = LONDON;
            const end = new Coordinate(LONDON.latitude + 0.0001, LONDON.longitude + 0.0001);
            const arc = Arc.fromPoints(start, end);
