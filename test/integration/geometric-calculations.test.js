...
import { LONDON, NEW_YORK, TOKYO } from '../__fixtures__/coordinates.js';
describe('Geometric Calculations Integration', () => {
    describe('Flight Route Planning', () => {
        it('should calculate great circle route with waypoints', () => {
            ...
            const totalDistance = new Distance(leg1.distance().inMeters() + leg2.distance().inMeters());
        ...
        it('should handle flight path restrictions', () => {
            ...
            const restrictedZone = SmallCircle.withCenter(new Coordinate(50.0, 40.0)).radius(Distance.fromKilometers(500));
    ...
    describe('Maritime Navigation', () => {
        ...
        it('should handle emergency diversion scenarios', () => {
            ...
            // Calculate diversion
            const diversionRoute = GreatCircle.from(originalRoute.interpolate(0.3)).to(halifax);
            // Calculate new total distance
            const distanceToEmergency = diversionRoute.distance();
            // Verify reasonable results
            expect(distanceToEmergency.inKilometers()).toBeGreaterThan(0);
            expect(distanceToEmergency.inKilometers()).toBeLessThan(originalRoute.distance().inKilometers());
        });
    });
    ...
    describe('Survey Route Planning', () => {
        it('should generate parallel survey lines', () => {
            ...
            const totalDistance = lines.reduce((sum, line) => sum + line.distance().inMeters(), 0);
