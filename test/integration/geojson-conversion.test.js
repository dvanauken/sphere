...
import { LONDON, PARIS, NEW_YORK, TOKYO } from '../__fixtures__/coordinates.js';
describe('GeoJSON Integration', () => {
    describe('Route Planning Data Exchange', () => {
        it('should convert complex route to GeoJSON and back', () => {
            ...
            const reconvertedRoutes = features.map(feature => TypeConverter.fromFeature(feature));
        ...
        it('should preserve route properties in conversion', () => {
            ...
            expect(feature.properties.distance).toBeCloseTo(distance.inMeters(), -2);
            // Convert back and verify
            const reconverted = TypeConverter.fromFeature(feature);
            expect(reconverted.distance().inMeters()).toBeCloseTo(distance.inMeters(), -2);
        });
    });
    describe('Area Coverage Export/Import', () => {
        ...
        it('should handle collection of search areas', () => {
            ...
            for (let i = 0; i < 3; i++) {
                const center = new Coordinate(baseCenter.latitude + (i * 0.01), baseCenter.longitude + (i * 0.01));
                areas.push(SmallCircle.withCenter(center)
                    .radius(Distance.fromKilometers(5)));
            }
            // Convert all to GeoJSON
            const features = areas.map(area => TypeConverter.toFeature(area));
            // Convert back
            const reconverted = features.map(feature => TypeConverter.fromFeature(feature));
    ...
    describe('Data Validation and Error Handling', () => {
        it('should handle invalid GeoJSON input', () => {
            ...
            invalidFeatures.forEach(feature => {
                expect(() => TypeConverter.fromFeature(feature)).toThrow();
            });
        });
    ...
    describe('Complex Data Structures', () => {
        it('should handle nested geometry collections', () => {
            ...
            const features = searchPatterns.map(pattern => TypeConverter.toFeature(pattern));
            // Convert back
            const reconverted = features.map(feature => TypeConverter.fromFeature(feature));
    ...
    describe('Batch Processing', () => {
        it('should handle bulk conversions', () => {
            ...
            const geometries = [
                ...
                SmallCircle.withCenter(LONDON).radius(Distance.fromKilometers(10)),
        ...
        it('should handle collection conversion', () => {
            ...
            const reconvertedRoutes = collection.features.map(feature => TypeConverter.fromFeature(feature));
            expect(reconvertedRoutes.length).toBe(routes.length);
            reconvertedRoutes.forEach(route => {
                expect(route).toBeInstanceOf(GreatCircle);
            });
